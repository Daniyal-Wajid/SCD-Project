...
export default function BookEvent() {
  // Dummy business services for selection (replace with backend fetch)
  const [businessServices, setBusinessServices] = useState([
    { id: 1, type: "Hall", name: "Grand Ballroom" },
    { id: 2, type: "Catering", name: "Gourmet Catering" },
    { id: 3, type: "Decor", name: "Floral Designs" },
    { id: 4, type: "Food", name: "Delicious Bites" },
  ]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // For now, just alert form data - later send to backend
    alert(`Booking Submitted!
Event: ${formData.eventName}
Date: ${formData.eventDate}
Services: ${Object.entries(formData.selectedServices)
  .filter(([key, val]) => val)
  .map(([key, val]) => `${key}: ${val}`)
  .join(", ")}
